...
USE_IMGUIZMO=-I../ImGuizmo-master/ ../ImGuizmo-master/ImGuizmo.cpp
...
$(OUTPUT): $(SOURCES)
	$(CC) -Isrc -I/usr/local/include/ $(SOURCES) $(USE_IMGUI) $(USE_IMPLOT) -std=c++1z $(USE_VORBIS) $(USE_LIBPNG) $(USE_WEBGL) $(USE_ENGINE_WEBCORE) $(LDFLAGS) -o $(OUTPUT) -s WASM=1 -s ASSERTIONS=1 --preload-file data -s ALLOW_MEMORY_GROWTH=1 -s FETCH=1 --emrun $(USE_IMGUIZMO) -s DISABLE_EXCEPTION_CATCHING=0
